...
function co2lab3DPUMLE(varargin)
    ...
    if nargin ~= numel(param_names)
        error('Usage: co2lab3DPUMLE(param1, param2, ..., paramN) with all required parameter files.');
    end
    
    fprintf('Loading parameter files:\n');

    for s = 1:length(param_names), PARAMS.(param_names{s}) = load(varargin{s}); end

    fprintf('[EXECUTION] PUMLE''s .mat files loaded for simulation.\n')

    %% General Settings
    % Run MRST startup (for command line)
    run(fullfile(PARAMS.EXECUTION.mrst_root,'startup.m'));
    ...
    % check fields in the grdecl structure
    ...
    disp('Cartesian dimensions:');
    disp(G.cartDims);
    
    rock = grdecl2Rock(grdecl, G.cells.indexMap);
    
    % For each faultdata string, if it begins with a quantifier,
    % insert a neutral token (here, a character class matching letters)
    if isfield(rock, 'faultdata')
        if iscell(rock.faultdata)
            for i = 1:numel(rock.faultdata)
                fd = rock.faultdata{i};
                if ~isempty(fd) && ismember(fd(1), {'*','+','?'})
                    % Insert a neutral token at the beginning.
                    % Adjust the token if fault names are expected to start differently.
    ...
    % Check if the rock model has faults
    ...
    fprintf('[EXECUTION] Reference pressure: %.2f MPa\n', P_r)
    fprintf('[EXECUTION] Type of reference pressure: %s\n', class(P_r));
    ...
    % W(1).val = 1.5527e-06; % Set the control value for well 1
    ...
    %% Simulate
    disp(class(initState))
    disp(fieldnames(initState))
    [wellSol, states] = simulateScheduleAD(initState, model, schedule, 'Verbose', true);
