...
P_r = str2double(PARAMS.Fluid.pres_ref); % reference pressure [MPa]
T_r = str2double(PARAMS.Fluid.temp_ref); % reference temperature [º C]
%% Brine saturation
...
X_NaCl = str2double(PARAMS.Fluid.XNaCl);
X_H2O = 1 - X_NaCl; % 
...
cf_rock = str2double(PARAMS.Fluid.cp_rock) / barsa; % rock compressibility
muw     = str2double(PARAMS.Fluid.mu_brine) * Pascal * second; % brine viscosity
...
initState.s = repmat([str2double(PARAMS.InitialConditions.sw_0), 1 - str2double(PARAMS.InitialConditions.sw_0)],...
...
dT = rampupTimesteps(str2double(PARAMS.Schedule.injection_time) * year, ...
    str2double(PARAMS.Schedule.injection_timestep_rampup) * year, 4);  % injection with increasing timestep size

schedule.step.val = [dT; ... 
                    repmat(str2double(PARAMS.Schedule.migration_time) * year, ...
                        str2double(PARAMS.Schedule.migration_timestep), 1)]; % post injection
