...
base_chema = pa.DataFrameSchema({
    ...
    "initial_conditions__sw_0": pa.Column(float, checks=pa.Check.in_range(0, 1), nullable=False),
    "boundary_conditions__type": pa.Column(str, nullable=False),
    "wells__co2_inj": pa.Column(float, checks=pa.Check.gt(0), nullable=False),
    "schedule__injection_time": pa.Column(int, checks=pa.Check.gt(0), nullable=False),
    "schedule__migration_time": pa.Column(int, checks=pa.Check.gt(0), nullable=False),
    "schedule__injection_timesteps": pa.Column(int, checks=pa.Check.gt(0), nullable=False),
    "schedule__migration_timesteps": pa.Column(int, checks=pa.Check.gt(0), nullable=False),
})
...
class Metadata:
    ...
    def get_data(self, **kwargs) -> None:
        self.parameters = kwargs.get("parameters") if self.parameters is None else self.parameters
        self.base_schema = kwargs.get("base_schema") if self.base_schema is None else self.base_schema
        self.dimensions = kwargs.get("dimensions") if self.dimensions is None else self.dimensions
        self.timestamps = kwargs.get("timestamps") if self.timestamps is None else self.timestamps
    ...
    def save_bronze_data(self) -> None:
        self.to_data_frame()
        self.parameters.reset_index().to_csv(os.path.join(self.path, "bronze_metadata.csv"))
    ...
    def _cast_base_cols(self):
        ...
        self._cast_columns(
            self.parameters,
            ["schedule__injection_time", "schedule__migration_time", "schedule__injection_timesteps", "schedule__migration_timesteps"],
            int,
        )
        self._cast_columns(
            self.parameters,
            ["fluid__pres_ref", "fluid__temp_ref", "fluid__cp_rock", "fluid__srw", "fluid__src", "fluid__pe", "fluid__xnacl", "fluid__rho_h2o", "initial_conditions__sw_0", "wells__co2_inj"],
            float,
        )

    def clean_parameters(self) -> None:
        columns_to_parse = {"Fluid", "Initial Conditions", "Boundary Conditions", "Wells", "Schedule", "SimNums"}
        columns_to_drop = ["Paths", "Pre-Processing", "Grid", "Fluid", "Initial Conditions", "Boundary Conditions", "Wells", "Schedule", "EXECUTION", "SimNums"]
        for main_field, (child_fields, _) in self.base_schema.items():
            ...
            values_extracted = values if isinstance(values[0], dict) else list(map(eval, values))
            for child_field in child_fields:
                ...
                if child_field == self.parameters_id:
                    ...
                    self.parameters[self._format_column_name(main_field, child_field)] = values
    ...
    def add_dimensions(self) -> None:
        ...
        self._cast_columns(self.parameters, ["dimension_x", "dimension_y", "dimension_z"], int)
        dimensions_schema = {
